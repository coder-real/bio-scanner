...
const char* SERVER_IP   = "10.158.59.28";
...
void setup() {
  ...
  Serial.println("[ESP] Starting WiFiManager...");
  ...
  if (MDNS.begin("biocolour")) {
    Serial.println("[ESP] mDNS started");
  }
}
...
void sendHeartbeat() {
  WiFiClient client;
  HTTPClient http;
  String url = "http://" + String(SERVER_IP) + ":" + String(SERVER_PORT) + "/api/heartbeat";

  Serial.println("[ESP] Sending Heartbeat...");
    ...
    if (line.startsWith("DATA:")) {
      ...
      if (pipeIdx != -1) {
        ...
        if (comma1 != -1 && comma2 != -1) {
          ...
          WiFiClient client;
          HTTPClient http;
          String url = "http://" + String(SERVER_IP) + ":" + String(SERVER_PORT) + "/api/reading";

          Serial.println("[ESP] POSTing data...");
          http.begin(client, url);
          http.addHeader("Content-Type", "application/json");
          ...
          if (httpCode == 200) {
            ...
            lastHeartbeat = millis(); // Reset heartbeat timer since we just sent data
          } else {
